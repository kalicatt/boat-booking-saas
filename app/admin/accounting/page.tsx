      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-xl border bg-white p-4 lg:col-span-2">
          ...
          <table className="w-full text-sm">
            <thead><tr><th className="p-2">Date</th><th className="p-2">Type</th><th className="p-2">Provider</th><th className="p-2">Method</th><th className="p-2">Montant</th><th className="p-2">Booking</th></tr></thead>
            <tbody>
              {(ledger||[]).map((e:any)=> (
            ...
            {(closures||[]).map((c:any)=>{
              ...
              return (
                <tr key={c.id} className="border-t">
                  ...
                  <td className="p-2">{Object.entries(snapshot.totals).map(([k,v])=> `${k}: ${(Number(v)/100).toFixed(2)}€`).join(' • ')} | Vouchers: {Object.entries(snapshot.vouchers).map(([k,v])=> `${k}: ${Number(v)}`).join(' • ')}</td>
                </tr>
              )
            })}
          </tbody>
