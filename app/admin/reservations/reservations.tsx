  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="border rounded px-3 py-2 bg-blue-600 text-white hover:bg-blue-700" onClick={()=>setShowCreate(true)}>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 flex flex-wrap gap-3 items-end">
        ...
        <div className="text-xs text-slate-500">Astuce: sélectionnez une ligne comme base horaire puis prévisualisez.</div>
      </div>
            ...
            <tbody>
              {bookings.map((b:any)=>(
                <tr key={b.id} className={`border-b hover:bg-slate-50 ${selectedId===b.id?'bg-yellow-50':''}`} onClick={()=>setSelectedId(b.id)}>
                  ...
                  <td className="p-3">{(b.payments?.[0]?.provider || '—')}{b.payments?.[0]?.methodType ? ` (${b.payments[0].methodType})` : ''}</td>
                  <td className="p-3">
                    ...
                    <button className="border rounded px-2 py-1" onClick={async (e)=>{ 
                      e.stopPropagation(); 
                      const ok = window.confirm('Confirmer la suppression de cette réservation ?');
                      if (!ok) return;
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            {chainPreview.length === 0 ? (
              <div className="text-sm text-slate-500">Renseignez un nombre de personnes et cliquez sur « Prévisualiser chaîne ».</div>
            ) : (
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <label className="flex items-start gap-2 text-sm">
              ...
              <span>
                ...
                <span className="block text-xs text-slate-500 mt-1">Copie les informations de paiement (fournisseur, type) de la réservation sélectionnée vers les réservations créées par la chaîne. Les paiements restent marqués comme « en attente » tant qu’ils ne sont pas confirmés.</span>
              </span>
            </label>
            <button
              ...
              onClick={async ()=>{
                ...
                mutate()
                if (createdCount>0) setToasts(t=>[...t, { id: Date.now(), type:'success', message:`Chaîne créée: ${createdCount} créées, ${overlapCount} en conflit` }])
                else setToasts(t=>[...t, { id: Date.now(), type:'warning', message:`Aucune création, ${overlapCount} conflits` }])
              }}
            >Créer la chaîne</button>
          </div>
          {chainResult && (
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <div className="text-sm font-semibold mb-2">Résultat de la chaîne</div>
              <div className="text-sm">Créées: <span className="font-medium">{chainResult.created}</span> • Conflits: <span className="font-medium">{chainResult.overlaps}</span></div>
              {(chainResult.details && chainResult.details.length>0) && (
                <ul className="mt-2 space-y-1 text-xs text-slate-600">
                  {chainResult.details.map((d:any, i:number)=> (
                    <li key={i}>#{d.index} {d.start}→{d.end} — {d.reason}</li>
                  ))}
                </ul>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded shadow p-4 w-full max-w-xl">
            ...
            <div className="space-y-3">
              ...
              {createTab==='contact' && (
                <div>
                  ...
                  <button className="border rounded px-3 py-1 bg-blue-600 text-white disabled:opacity-50" disabled={!selectedContactId} onClick={async ()=>{
                    ...
                    if (r.ok) { setShowCreate(false); mutate(); setToasts(t=>[...t, { id: Date.now(), type:'success', message:'Contact converti en réservation' }]) }
                  }}>Convertir</button>
                </div>
              )}
              {createTab==='group' && (
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button className="border rounded px-3 py-1 bg-blue-600 text-white" disabled={creating} onClick={async ()=>{
                const bodyBase = {
                  date: form.date,
                  time: form.time,
                  adults: form.adults,
                ...
                if (createTab==='private') {
                  resp = await fetch('/api/bookings', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ...bodyBase, private: true }) })
                } else if (createTab==='group') {
                  resp = await fetch('/api/bookings', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ...bodyBase, groupChain, inheritPaymentForChain }) })
                } else {
                  resp = await fetch('/api/bookings', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(bodyBase) })
                }
                setCreating(false)
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4 flex flex-wrap gap-3 items-end">
              ...
              <div className="text-xs text-slate-500">
                Astuce: sélectionnez une ligne comme base horaire puis prévisualisez.
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={()=>setShowEdit(null)}>
          <div className="bg-white rounded shadow p-4 w-full max-w-xl" onClick={e=>e.stopPropagation()}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button className="border rounded px-3 py-1 bg-blue-600 text-white" onClick={()=>{ const ok = window.confirm('Confirmer la modification de cette réservation ?'); if (ok) { /* TODO: wire PATCH */ setShowEdit(null); } }}>Enregistrer</button>
            </div>
          </div>
        </div>
      )}
