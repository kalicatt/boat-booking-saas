...
import { format, parse, startOfWeek, getDay, startOfDay, endOfDay, addMinutes, areIntervalsOverlapping, isPast } from 'date-fns'
...
export default function AdminPlanning() {
  ...
  // Clé pour forcer le rafraîchissement après un CRUD manuel
  ...
  /**
   * Fonction de fetch des bookings (ne dépend plus de useCallback car la dépendance à currentRange est gérée par useEffect)
   */
    ...
    try {
      ...
      const formattedEvents = rawData.map((b: any) => {
        ...
        return {
          ...
          resourceId: b.boatId, 
          clientName: fullName,
          peopleCount: b.peopleCount, // Correction: utilisation du champ direct
          boatCapacity: b.boat.capacity,
          totalOnBoat: loadMap[key] || 0,
          user: b.user,
          language: b.language,
  ...
  // 1. Chargement initial des barques
  useEffect(() => {
    fetchBoats();
  }, [fetchBoats]);

  /**
   * 2. Chargement des Bookings & Polling
   * Cet effet gère le fetch et l'intervalle. Il se nettoie et se recrée SEULEMENT quand currentRange change.
   */
  useEffect(() => {
      // Fonction à appeler par l'intervalle
      const autoRefresh = () => {
          fetchBookings(currentRange.start, currentRange.end);
      };

      // Premier appel immédiat (au montage ou changement de range/refreshKey)
      autoRefresh();
      
      // Polling : on le crée en dépendant de currentRange
      const intervalId = setInterval(autoRefresh, 10000); 

      // Cleanup : détruit l'ancien intervalle lorsque currentRange change (pour éviter les doublons/stales)
      return () => clearInterval(intervalId);
      
  }, [currentRange, refreshKey]); // Dépend de currentRange et refreshKey (pour les actions manuelles)
  ...
  /**
   * Mise à jour du range (déclenche un re-fetch via useEffect)
   */
  const handleRangeChange = (range: Date[] | { start: Date; end: Date }) => {
    ...
    // Mise à jour de l'état currentRange -> Déclenchement du useEffect
  ...
  const handleQuickBookingSuccess = () => {
      ...
      // Incrémente la clé pour forcer le useEffect à re-fetch immédiatement
      setRefreshKey(prev => prev + 1); 
  };


  // --- 4. ACTIONS CRUD (Définies ici pour être passées à la modale) ---
  ...
  const handleStatusUpdate = async (id: string, newCheckinStatus?: string, newIsPaid?: boolean) => {
    ...
    try {
        ...
        if (res.ok) {
            // Après succès, on force le refresh
            setRefreshKey(prev => prev + 1); 
  ...
  const EventComponent = ({ event }: { event: BookingDetails }) => {
    ...
    // NOUVEAU: Définition de la couleur du dot de paiement
      ...
      <div className="flex-1 bg-white border rounded-xl shadow-sm p-4">
        ...
        {!loadingBoats && resources.length === 0 ? (
            ...
            eventPropGetter={(event: any) => {
                 ...
                 // COULEURS BASÉES SUR LE STATUT CHECK-IN
                 if (event.checkinStatus === 'EMBARQUED') {
                     style.backgroundColor = '#1f4068'; // Bleu Profond
                     style.color = 'white';
                 } else if (event.checkinStatus === 'NO_SHOW') {
                     style.backgroundColor = '#e69900'; // Jaune/Orange Sauf
                     style.color = 'black';
                 } else {
                     // Couleur de la barque par défaut
