...
async function main() {
  ...
  const outputs: Array<{ file: string; size: number; maskable?: boolean }> = [
  ...
  for (const out of outputs) {
    ...
    const img = sharp(src).resize({ width: Math.round(size * (maskable ? 0.76 : 0.9)), height: Math.round(size * (maskable ? 0.76 : 0.9)), fit: 'inside' })
